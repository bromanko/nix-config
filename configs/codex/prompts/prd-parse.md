...
1. **Verify PRD exists:**
   ...
   - If not found, tell user: "❌ PRD not found: $ARGUMENTS. First create it with: /prd-new $ARGUMENTS"
...
3. **Check for existing epic:**
   ...
   - If user says no, suggest: "View existing epic with: /epic-show $ARGUMENTS"
...
2. Show summary of:
   ...
   - Estimated effort
3. Suggest next step: "Ready to break down into tasks? Run: /epic-decompose $ARGUMENTS"
...
1. **Verify <feature_name> was provided as a parameter:**
   - If not, tell user: "❌ <feature_name> was not provided as parameter. Please run: /prd-parse <feature_name>"
...
### 1. Read the PRD
- Use `shell` commands (for example `cat` or `sed -n`) to load `.claude/prds/$ARGUMENTS.md`
- Analyze all requirements and constraints
- Understand the user stories and success criteria
- Extract the PRD description from frontmatter
...
### 3. File Format with Frontmatter
Create the epic file at: `.claude/epics/$ARGUMENTS/epic.md` with this exact structure (rewrite the file using `shell` redirection to avoid partial edits):
